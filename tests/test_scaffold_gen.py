...
from agents.scaffold_gen import (
    generate_intent_scaffold,
    generate_req_scaffold,
    generate_uvm_tb,
    _sv_file_header,
)
...
# ===========================================================================
# STD-006 header tests (Tasks 3-5)
# ===========================================================================

def test_sv_header_fields() -> None:
    """_sv_file_header() produces all required STD-006 fields."""
    header = _sv_file_header(
        filename="test_file.sv",
        brief="Test brief",
        functional_blocks=[("Block A", "Does A")],
    ...
    assert "pssgen" in header
    assert "SystemVerilog IEEE 1800-2017" in header
    assert "generated" in header
    assert "Generated by scaffold_gen.generate_uvm_tb()" in header
    assert header.startswith("// ==")


def test_generated_files_have_headers(tmp_path) -> None:
    """All generated .sv files start with the STD-006 header; build.tcl starts with # ==."""
    ir = _make_uvm_ir()
    generate_uvm_tb(ir, None, str(tmp_path))
    tb_dir = os.path.join(str(tmp_path), "tb")

    # Spot-check: driver and interface must start with // ==
    driver = open(os.path.join(tb_dir, f"{ir.design_name}_driver.sv"),
                  encoding="utf-8").read()
    assert driver.startswith("// =="), "driver.sv must start with STD-006 header"

    intf = open(os.path.join(tb_dir, f"{ir.design_name}_if.sv"),
                encoding="utf-8").read()
    assert intf.startswith("// =="), "if.sv must start with STD-006 header"

    # Tcl build script must start with # ==
    tcl = open(
        os.path.join(tb_dir, "scripts", "vivado", "build.tcl"),
        encoding="utf-8",
    ).read()
    assert tcl.startswith("# =="), "build.tcl must start with STD-006 Tcl header"
